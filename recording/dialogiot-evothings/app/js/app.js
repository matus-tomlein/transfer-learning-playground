...
(function(){
  ...
  // evothings.loadScript('js/mqttws31.js');
  evothings.loadScript('js/mqtt.min.js');
  evothings.loadScript('js/logging.js');
  ...
  app.initialize = function()
  {
    ...
    iotsensor
      .accelerometerCallback(logging.logAccelerometerData)
      .gyroscopeCallback(logging.logGyroscopeData)
      .magnetometerCallback(logging.logMagnetometerData)
      .barometerCallback(logging.logBarometerData)
      .temperatureCallback(logging.logTemperatureData)
      .humidityCallback(logging.logHumidityData)
      .sflCallback(logging.logSensorFusionData)
  ...
  app.publish = function(data) {
    data.device = 'DialogIoT ' + app.currentDeviceAddress;
    if (app.connection) {
      app.connection.send(JSON.stringify(data));
    } else {
      console.log('Not connected, cant publish the message');
    }
  };
  ...
  app.onConnectButton = function(address)
  {
    if(iotsensor.isIoTSensor(devices[address]))
    {
      ...
      iotsensor.connectToDevice(
          ...
          {
            connected = 'true';
            app.currentDeviceAddress = address.hashCode();
